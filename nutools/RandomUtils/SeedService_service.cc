 ...
 * @see SeedService.h SeedMaster.h
 */

// SeedService header
#include "nutools/RandomUtils/SeedService.h"
...
namespace rndm {
    ...
    , bPrintEndOfJobSummary(paramSet.get<bool>("endOfJobSummary",false))
  {
    ...
    iRegistry.sPreProcessEvent.watch        (this, &SeedService::preProcessEvent        );
    iRegistry.sPreModule.watch              (this, &SeedService::preModule              );
    iRegistry.sPostModule.watch             (this, &SeedService::postModule             );
    iRegistry.sPostProcessEvent.watch       (this, &SeedService::postProcessEvent       );
    iRegistry.sPreModuleEndJob.watch        (this, &SeedService::preModuleEndJob        );
    iRegistry.sPostModuleEndJob.watch       (this, &SeedService::postModuleEndJob       );
  ...
  //----------------------------------------------------------------------------
  SeedService::EngineId SeedService::qualify_engine_label
  ...
  //----------------------------------------------------------------------------
  SeedService::seed_t SeedService::getSeed(std::string instanceName) {
    return getSeed(qualify_engine_label(instanceName));
  } // SeedService::getSeed(string)
  
  
  //----------------------------------------------------------------------------
  SeedService::seed_t SeedService::getGlobalSeed(std::string instanceName) {
    EngineId ID(instanceName, EngineId::global);
    LOG_DEBUG("SeedService")
      << "SeedService::getGlobalSeed(\"" << instanceName << "\")";
    return getSeed(ID);
  } // SeedService::getGlobalSeed()
  
  
  //----------------------------------------------------------------------------
  SeedService::seed_t SeedService::getSeed(EngineId const& id) {
  ...
  } // SeedService::getSeed(EngineId)
  
  
  SeedService::seed_t SeedService::querySeed(EngineId const& id) {
    return seeds.getSeed(id); // ask the seed to seed master
  } // SeedService::querySeed()
  
  
  std::pair<SeedService::seed_t, bool> SeedService::findSeed(
    EngineId const& id,
  ...
  } // SeedService::findSeed()
  
  
  //----------------------------------------------------------------------------
  SeedService::seed_t SeedService::createEngine(
    ...
    mf::LogInfo("SeedService")
      << "Seeding " << type << " engine \"" << id.artName()
      << "\" with seed " << seed << ".";
    return seed;
  } // SeedService::createEngine(strings)
  
  
  SeedService::seed_t SeedService::createEngine(art::EngineCreator& module) {
  ...
  } // SeedService::createEngine(ParameterSet)
  ...
  SeedService::seed_t SeedService::registerEngine(
    ...
    return seed;
  } // SeedService::registerEngine(Seeder_t, string, ParameterSet, init list)
  
    
  SeedService::seed_t SeedService::declareEngine(std::string instance) {
    return registerEngine(SeedMaster_t::Seeder_t(), instance);
  } // SeedService::declareEngine(string)
  
  
  SeedService::seed_t SeedService::declareEngine(
    std::string instance,
    fhicl::ParameterSet const& pset, std::initializer_list<std::string> pnames
  ) {
    return registerEngine(SeedMaster_t::Seeder_t(), instance, pset, pnames);
  } // SeedService::declareEngine(string, ParameterSet, init list)
  
  
  SeedService::seed_t SeedService::defineEngine
    (SeedMaster_t::Seeder_t seeder, std::string instance /* = {} */)
  {
    return defineEngineID(qualify_engine_label(instance), seeder);
  } // SeedService::defineEngine(string, Seeder_t)
  
    
  //----------------------------------------------------------------------------
  SeedService::seed_t SeedService::registerEngineID(
    EngineId const& id,
    SeedMaster_t::Seeder_t seeder /* = SeedMaster_t::Seeder_t() */
  ) {
    prepareEngine(id, seeder);
    return seedEngine(id);
  } // SeedService::registerEngine()
  
  
  SeedService::seed_t SeedService::defineEngineID
    (EngineId const& id, SeedMaster_t::Seeder_t seeder)
  {
    ...
    return seed;
  } // SeedService::defineEngineID()
  
  
  //----------------------------------------------------------------------------
  void SeedService::ensureValidState(bool bGlobal /* = false */) const {
    if (bGlobal) {
      ...
      {
        throw art::Exception(art::errors::LogicError)
          << "SeedService: not in a service constructor."
      ...
      {
        throw art::Exception(art::errors::LogicError)
          << "SeedService: not in a module constructor."
          << " May not register engines.\n";
      }
    } // if
  } // SeedService::ensureValidState()
  
  
  //----------------------------------------------------------------------------
  SeedService::seed_t SeedService::reseedInstance(EngineId const& id) {
    ...
    if (seed == InvalidSeed) {
      mf::LogDebug("SeedService")
        << "No random seed specific to this event for engine '" << id << "'";
    }
    else {
      mf::LogInfo("SeedService") << "Random seed for this event, engine '"
        << id << "': " << seed;
    }
    return seed;
  } // SeedService::reseedInstance()
  
  
  void SeedService::reseedModule(std::string currentModule) {
    for (EngineId const& ID: seeds.engineIDsRange()) {
      if (ID.moduleLabel != currentModule) continue; // not our module? neeext!!
      reseedInstance(ID);
    } // for
  } // SeedService::reseedModule(string)
  
  void SeedService::reseedModule() { reseedModule(state.moduleLabel()); }
  
  
  void SeedService::reseedGlobal() {
    for (EngineId const& ID: seeds.engineIDsRange()) {
      if (!ID.isGlobal()) continue; // not global? neeext!!
      reseedInstance(ID);
    } // for
  } // SeedService::reseedGlobal()
  
  
  //----------------------------------------------------------------------------
  void SeedService::RandomNumberGeneratorSeeder
  ...
  } // SeedService::RandomNumberGeneratorSeeder()
  
  
  //----------------------------------------------------------------------------
  void SeedService::registerEngineAndSeeder
    (EngineId const& id, SeedMaster_t::Seeder_t seeder)
  {
    ...
    if (hasEngine(id)) {
      throw art::Exception(art::errors::LogicError)
        << "SeedService: an engine with ID '" << id.artName()
        << "' has already been created!\n";
    }
    seeds.registerNewSeeder(id, seeder);
  } // SeedService::registerEngineAndSeeder()
  
  
  //----------------------------------------------------------------------------
  void SeedService::freezeSeed(EngineId const& id, seed_t frozen_seed)
    { seeds.freezeSeed(id, frozen_seed); }
  
  
  //----------------------------------------------------------------------------
  SeedService::seed_t SeedService::prepareEngine
    (EngineId const& id, SeedMaster_t::Seeder_t seeder)
  {
    registerEngineAndSeeder(id, seeder);
    return querySeed(id);
  } // SeedService::prepareEngine()
  
  
  //----------------------------------------------------------------------------
  bool SeedService::readSeedParameter(
  ...
  void SeedService::preModuleConstruction(art::ModuleDescription const& md)  {
    ...
    state.set_module(md);
  } // SeedService::preModuleConstruction()
  
  void SeedService::postModuleConstruction(art::ModuleDescription const&) {
    state.reset_state();
  } // SeedService::postModuleConstruction()
  ...
  void SeedService::preModuleBeginRun(art::ModuleDescription const& md) {
    ...
    state.set_module(md);
  } // SeedService::preModuleBeginRun()
  
  void SeedService::postModuleBeginRun(art::ModuleDescription const&) {
    state.reset_state();
  } // SeedService::postModuleBeginRun()
  
  void SeedService::preProcessEvent(art::Event const& evt) {
    state.transit_to(SeedServiceHelper::ArtState::inEvent);
    state.set_event(evt);
    seeds.onNewEvent(); // inform the seed master that a new event has come
    
    LOG_DEBUG("SeedService") << "preProcessEvent(): will reseed global engines";
    reseedGlobal(); // why don't we do them all?!?
    
  } // SeedService::preProcessEvent()
  
  void SeedService::preModule(art::ModuleDescription const& md) {
    ...
    LOG_DEBUG("SeedService") << "preModule(): will reseed engines for module '"
      << md.moduleLabel() << "'";
    reseedModule(md.moduleLabel());
  } // SeedService::preModule()
  
  void SeedService::postModule(art::ModuleDescription const&) {
    state.reset_module();
    state.reset_state();
  } // SeedService::postModule()
  
  void SeedService::postProcessEvent(art::Event const&) {
    state.reset_event();
    state.reset_state();
  } // SeedService::postProcessEvent()
  
  void SeedService::preModuleEndJob(art::ModuleDescription const& md) {
    state.transit_to(SeedServiceHelper::ArtState::inEndJob);
    state.set_module(md);
  } // SeedService::preModuleBeginRun()
  
  void SeedService::postModuleEndJob(art::ModuleDescription const&) {
    state.reset_state();
  } // SeedService::preModuleBeginRun()
  ...
  } // SeedService::postEndJob()
  
  //----------------------------------------------------------------------------

} // end namespace rndm

DEFINE_ART_SERVICE(rndm::SeedService)
